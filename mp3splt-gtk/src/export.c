...
static gpointer export_cue_in_configuration_directory_for_thread(ui_with_pat *ui_pat)
{
  ...
  if (ui_pat->previous_thread) { g_thread_join(ui_pat->previous_thread); }

  set_process_in_progress_and_wait_safe(TRUE, ui);

  mp3splt_set_int_option(
    ui->mp3splt_state, SPLT_OPT_CUE_DISABLE_CUE_FILE_CREATED_MESSAGE, SPLT_TRUE);
  ...
  mp3splt_set_int_option(
    ui->mp3splt_state, SPLT_OPT_CUE_DISABLE_CUE_FILE_CREATED_MESSAGE, SPLT_FALSE);
...
void export_cue_file_in_configuration_directory(ui_state *ui)
{
  ...
  else { ui_pat->previous_thread = NULL; }

  ui->infos->previous_export_thread = create_thread(
    (GThreadFunc)export_cue_in_configuration_directory_for_thread, ui_pat, ui, "export_cue_auto");
}
