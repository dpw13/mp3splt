...
void set_input_filename(const gchar *filename, ui_state *ui)
{
  ...
  if (ui->gui->input_filename != NULL) { g_string_free(ui->gui->input_filename, TRUE); }
...
{
  if (gui->input_filename != NULL) { return gui->input_filename->str; }
...
static void activate_url(GtkAboutDialog *about, const gchar *link, ui_state *ui)
{
  ...
  if (default_browser[0] != '\0')
  {
    ...
    snprintf(browser_exe_registry, 1024,
      "SOFTWARE\\Clients\\StartMenuInternet\\%s\\shell\\open\\command\\", default_browser);

    SHGetValue(
      HKEY_LOCAL_MACHINE, TEXT(browser_exe_registry), TEXT(""), &dwType, browser_exe, &dwSize);
    ...
    if (browser_exe[0] != '\0')
    {
      ...
      if (browser_command)
      {
        snprintf(browser_command, browser_command_size, "%s %s", browser_exe, link);
        ...
        if (!CreateProcess(NULL, browser_command, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pinf))
        {
...
static void about_window(GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ...
  g_snprintf(
    b3, 100, "-%s-\n%s libmp3splt %s", _("release of " MP3SPLT_GTK_DATE), b1, library_version);
  ...
  gtk_about_dialog_set_website_label(GTK_ABOUT_DIALOG(dialog), "http://mp3splt.sourceforge.net");
  gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(dialog), "http://mp3splt.sourceforge.net");

  gtk_about_dialog_set_translator_credits(
    GTK_ABOUT_DIALOG(dialog), "Mario Blättermann <mariobl@gnome.org>");
...
void cancel_button_event(GtkWidget *widget, ui_state *ui)
{
  ...
  if (widget != NULL) { gtk_widget_set_sensitive(widget, FALSE); }
...
{
  lock_mutex(&ui->variables_mutex);
  if (value) { ui->status->process_in_progress++; }
  else { ui->status->process_in_progress--; }
  unlock_mutex(&ui->variables_mutex);
}
...
void set_process_in_progress_and_wait_safe(gboolean value, ui_state *ui)
{
  if (value == TRUE)
  {
    while (get_process_in_progress_safe(ui)) { g_usleep(G_USEC_PER_SEC / 4); }
  }
...
void split_button_event(GtkWidget *widget, ui_state *ui)
{
  ...
  if (get_output_directory(ui) != NULL) { split_action(ui); }
  else { put_status_message(_(" error: no path of split selected"), ui); }
}

void set_split_file_mode(gint file_mode, ui_state *ui) { ui->infos->split_file_mode = file_mode; }

gint get_split_file_mode(ui_state *ui) { return ui->infos->split_file_mode; }
...
static void ShowHelp(GSimpleAction *action, GVariant *parameter, gpointer data)
{
  GError *gerror = NULL;
  gtk_show_uri(
    gdk_screen_get_default(), "ghelp:mp3splt-gtk", gtk_get_current_event_time(), &gerror);
}
#endif
...
static void player_seek_forward_action(GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ...
  gfloat new_time =
    ui->infos->current_time * 10 +
    (ui->infos->seek_jump_value != 0 ? ui->infos->seek_jump_value : 2. / 100. * total_time * 10);
...
static void player_big_seek_forward_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;

  gfloat total_time = ui->infos->total_time;
  gfloat new_time = ui->infos->current_time * 10 + (ui->infos->big_seek_jump_value != 0
                                                       ? ui->infos->big_seek_jump_value
                                                       : 15. / 100. * total_time * 10);
  if (new_time > total_time * 10) { new_time = total_time * 10; }
  player_seek(new_time, ui);
}

static void player_big_seek_backward_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void player_small_seek_forward_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void player_small_seek_backward_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void player_seek_to_next_splitpoint_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  if (time_right != -1) { player_seek(time_right * 10, ui); }
}

static void player_seek_to_previous_splitpoint_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  if (time_left != -1) { player_seek(time_left * 10, ui); }
}
...
static int find_closest_splitpoint(ui_state *ui)
{
  ...
  if (left_index_point == -1 && right_index_point == -1) { return -1; }
  ...
  if (time_to_right > time_to_left) { return left_index_point; }
...
static void player_seek_before_closest_splitpoint(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void player_seek_closest_splitpoint_no_pause(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void player_seek_closest_splitpoint(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void file_selection_changed(GtkFileChooser *open_file_chooser, ui_state *ui)
{
  ...
  if (previous_fname != NULL && filename != NULL && strcmp(filename, previous_fname) == 0)
  {
    return;
  }
...
static GtkWidget *create_choose_file_frame(ui_state *ui)
{
  GtkWidget *open_file_chooser_button =
    gtk_file_chooser_button_new(_("Open file ..."), GTK_FILE_CHOOSER_ACTION_OPEN);
  ...
  if (fname != NULL && strlen(fname) != 0)
  {
    gtk_file_chooser_set_filename(
      GTK_FILE_CHOOSER(open_file_chooser_button), get_input_filename(ui->gui));
  }
...
static GtkWidget *create_main_vbox(ui_state *ui)
{
  ...
  g_signal_connect(
    G_OBJECT(split_button), "clicked", G_CALLBACK(single_file_mode_split_button_event), ui);
...
static void move_and_resize_main_window(ui_state *ui)
{
  ...
  if (x != 0 && y != 0) { gtk_window_move(GTK_WINDOW(ui->gui->window), x, y); }
  else { gtk_window_set_position(GTK_WINDOW(ui->gui->window), GTK_WIN_POS_CENTER); }
...
static void parse_command_line_options(gint argc, gchar *argv[], ui_state *ui)
{
  ...
  if (!file_exists(argv[optind])) { ui_fail(ui, _("Cannot open input file %s\n"), argv[optind]); }
...
static void application_activate(GApplication *app, ui_state *ui)
{
  ...
  if (ui->infos->selected_player != PLAYER_GSTREAMER) { gtk_widget_hide(ui->gui->playlist_box); }
...
static void open_file_menu_action(GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ...
  GtkWidget *file_chooser =
    gtk_file_chooser_dialog_new(_("Choose File"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN, _("_Cancel"),
...
static void multiple_files_add_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
static void show_tracktype_window_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  if (ui->gui->freedb_window == NULL)
  {
    ui->gui->freedb_window =
      wh_create_window_with_close_button(_("TrackType"), 500, 300, GTK_WIN_POS_CENTER,
        GTK_WINDOW(ui->gui->window), ui->gui->freedb_widget, ui->gui->freedb_add_button, NULL);
  }
...
static void show_preferences_window_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  {
    ui->gui->preferences_window = wh_create_window_with_close_button(_("Preferences"), 750, 450,
      GTK_WIN_POS_CENTER, GTK_WINDOW(ui->gui->window), ui->gui->preferences_widget, NULL);
  }
...
static void show_splitpoints_window_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  {
    ui->gui->splitpoints_window = wh_create_window_with_close_button(_("Splitpoints"), 500, 300,
      GTK_WIN_POS_CENTER, GTK_WINDOW(ui->gui->window), ui->gui->splitpoints_widget,
...
static void show_split_files_window_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
  ...
  if (ui->gui->split_files_window == NULL)
  {
    ui->gui->split_files_window =
      wh_create_window_with_close_button(_("Split files"), 500, 300, GTK_WIN_POS_CENTER,
        GTK_WINDOW(ui->gui->window), ui->gui->split_files_widget, ui->gui->queue_files_button,
        ui->gui->remove_file_button, ui->gui->remove_all_files_button, NULL);
  }

  wh_show_window(ui->gui->split_files_window);
}

static void add_splitpoint_from_player_menu_action(
  GSimpleAction *action, GVariant *parameter, gpointer data)
{
  ui_state *ui = (ui_state *)data;
...
const static GActionEntry app_entries[] = { { "open_single_file", open_file_menu_action },
  ...
  { "preferences", show_preferences_window_menu_action }, { "split", split_menu_action },
  { "batch_split", batch_split_menu_action }, { "quit", exit_application_menu_action },

  { "view_splitpoints", show_splitpoints_window_menu_action },
  { "view_split_files", show_split_files_window_menu_action },

  { "pause_play", player_pause_action }, { "seek_forward", player_seek_forward_action },
  ...
  { "delete_closest_splitpoint", delete_closest_splitpoint }, { "zoom_in", zoom_in },
  { "zoom_out", zoom_out },
...
void put_status_message_in_idle(const gchar *text, ui_state *ui)
{
  ...
  add_idle(G_PRIORITY_HIGH_IDLE, (GSourceFunc)put_status_message_idle, ui_fname, NULL);
}
