...
static void dnd_data_received(GtkWidget *window, GdkDragContext *drag_context, gint x, gint y,
  GtkSelectionData *data, guint info, guint time, gboolean discard_data_files,
  gboolean discard_audio_files, gboolean single_file_mode, ui_state *ui);

static void dnd_data_received_data_files(GtkWidget *window, GdkDragContext *drag_context, gint x,
  gint y, GtkSelectionData *data, guint info, guint time, ui_state *ui)
{
...
static void dnd_data_received_batch_mode_audio_files(GtkWidget *window,
  GdkDragContext *drag_context, gint x, gint y, GtkSelectionData *data, guint info, guint time,
  ui_state *ui)
{
...
void dnd_add_drag_data_received_to_widget(GtkWidget *widget, drop_type type, ui_state *ui)
{
  ...
  switch (type)
  {
    ...
    case DND_DATA_FILES:
      g_signal_connect(
        G_OBJECT(widget), "drag-data-received", G_CALLBACK(dnd_data_received_data_files), ui);
      break;
  ...
  gboolean discard_audio_files, gboolean single_file_mode, ui_state *ui)
{
  ...
  while (current_filename != NULL)
  {
    ...
    else { filename = g_strdup(current_filename); }
    ...
    if (file_exists(filename))
    {
      ...
      if ((strstr(ext_str->str, ".MP3") != NULL) || (strstr(ext_str->str, ".OGG") != NULL) ||
          (strstr(ext_str->str, ".FLAC") != NULL))
      {
        filenames = g_slist_append(filenames, filename);
      }
      else if (!discard_data_files) { import_file(filename, ui, FALSE, TRUE); }
  ...
  if (single_file_mode)
  {
    ...
    import_file(last_filename->data, ui, FALSE, TRUE);

    g_slist_foreach(filenames, (GFunc)g_free, NULL);
    g_slist_free(filenames);
  }
  else { import_files_to_batch_and_free(filenames, ui); }
}
