...
static gboolean bus_call(GstBus *bus, GstMessage *msg, gpointer data)
{
  ...
  switch (GST_MESSAGE_TYPE(msg))
  {
    ...
    case GST_MESSAGE_TAG:
    {
      ...
      //artist
      const GValue *val = gst_tag_list_get_value_index(tag_list, GST_TAG_ARTIST, number_of_stream);
      if (val != NULL) { ui->pi->song_artist = g_value_get_string(val); }

      //title
      val = gst_tag_list_get_value_index(tag_list, GST_TAG_TITLE, number_of_stream);
      if (val != NULL) { ui->pi->song_title = g_value_get_string(val); }

      //rate (bps)
      val = gst_tag_list_get_value_index(tag_list, GST_TAG_BITRATE, number_of_stream);
      if (val != NULL) { ui->pi->rate = g_value_get_uint(val); }
...
void gstreamer_get_song_infos(gchar *total_infos, ui_state *ui)
{
  ...
  if (pad) { caps = gst_pad_get_current_caps(pad); }
...
gint gstreamer_get_playlist_number(ui_state *ui) { return 1; }
...
gchar *gstreamer_get_title_song(ui_state *ui)
{
  ...
  if (pi->song_artist || pi->song_title)
  {
    ...
    if (pi->song_artist) { title_size += strlen(pi->song_artist); }

    if (pi->song_title) { title_size += strlen(pi->song_title); }
...
{
  if (!ui->pi->play) { return 0; }

  GstQuery *query = gst_query_new_position(GST_FORMAT_TIME);
  gint64 time = 0;

  if (gst_element_query(ui->pi->play, query)) { gst_query_parse_position(query, NULL, &time); }
...
void gstreamer_add_files(GList *list, ui_state *ui)
{
  ...
  if (state == GST_STATE_PLAYING) { gst_element_set_state(ui->pi->play, GST_STATE_PLAYING); }
  else if (state == GST_STATE_PAUSED) { gst_element_set_state(ui->pi->play, GST_STATE_PAUSED); }
}
...
gint gstreamer_is_running(ui_state *ui) { return ui->pi->_gstreamer_is_running; }

//!returns TRUE if gstreamer is paused, if not, FALSE
gint gstreamer_is_paused(ui_state *ui)
{
  if (!ui->pi->play) { return FALSE; }
...
void gstreamer_play(ui_state *ui)
{
  ...
  GstState state;
  gst_element_get_state(ui->pi->play, &state, NULL, GST_CLOCK_TIME_NONE);
  if (state == GST_STATE_PLAYING) { gstreamer_jump(0, ui); }
...
void gstreamer_jump(gint position, ui_state *ui)
{
  ...
  gst_element_seek(ui->pi->play, 1.0, GST_FORMAT_TIME, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET,
    position * GST_MSECOND, 0, 0);
}
...
{
  if (ui->pi->play) { gst_element_set_state(ui->pi->play, GST_STATE_NULL); }
