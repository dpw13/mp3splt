...
{
  if (state->split.points && (index >= 0) && (index < state->split.points->real_splitnumber))
  {
    return SPLT_TRUE;
  }
...
{
  if (state->split.points == NULL) { return 0; }
...
int splt_sp_append_splitpoint(splt_state *state, long split_value, const char *name, int type)
{
  ...
  if (!split->points)
  {
    ...
    if (split->points == NULL) { return SPLT_ERROR_CANNOT_ALLOCATE_MEMORY; }
  ...
  {
    if ((split->points->points = realloc(split->points->points,
           (split->points->real_splitnumber + 1) * sizeof(splt_point))) == NULL)
    {
      return SPLT_ERROR_CANNOT_ALLOCATE_MEMORY;
    }
  }
...
splt_points *splt_sp_get_splitpoints(splt_state *state) { return state->split.points; }
...
void splt_sp_get_mins_secs_hundr_from_splitpoint(
  long splitpoint, long *mins, long *secs, long *hundr)
{
  if (hundr) { *hundr = splitpoint % 100; }

  splitpoint /= 100;

  if (secs) { *secs = splitpoint % 60; }

  if (mins) { *mins = splitpoint / 60; }
}

void splt_sp_get_mins_secs_frames_from_splitpoint(
  long splitpoint, long *mins, long *secs, long *frames)
{
  if (frames)
  {
...
{
  if (!state->split.points || (state->split.points->real_splitnumber <= 0)) { return; }
  ...
  for (i = 1; i < state->split.points->real_splitnumber; i++)
  {
    ...
    if (begin_point_type == SPLT_SKIPPOINT) { continue; }
...
splt_array *splt_sp_find_intervals_between_two_consecutive_big_tracks(
  splt_state *state, int min_track_join, int *error)
{
  splt_array *intervals = splt_array_new();
  if (!intervals) { return NULL; }

  if (!state->split.points || state->split.points->real_splitnumber == 1) { return intervals; }
  ...
  for (i = 1; i < state->split.points->real_splitnumber; i++)
  {
    ...
    if (track_length >= min_track_join)
    {
      ...
      {
        if (!fix_start_point) { start_interval = begin_index; }
        else
        {
    ...
    else
    {
      ...
      if (!appended_last && (i == state->split.points->real_splitnumber - 1 ||
                              i == state->split.points->real_splitnumber - 2))
      {
...
void splt_sp_join_minimum_tracks_splitpoints(splt_state *state, int *error)
{
  ...
  while (intervals && splt_array_length(intervals) > 0)
  {
    ...
    if (state->split.points->real_splitnumber == 2) { break; }
