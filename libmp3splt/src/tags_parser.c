...
static void splt_tp_process_tags(
  const char *tags, tags_parser_utils *tpu, splt_state *state, int *error);
static void splt_tp_process_tag_variable(const char *tag_variable_start,
  const char *end_paranthesis, tags_parser_utils *tpu, splt_state *state, int *error);
static void splt_tp_process_str_tags_variable(const char *tag_variable_start,
  const char *end_paranthesis, int tags_field, tags_parser_utils *tpu, splt_state *state,
  int *error);
static void splt_tp_process_tracknumber_variable(const char *tag_variable_start,
  const char *end_paranthesis, tags_parser_utils *tpu, splt_state *state, int *error);
static void splt_tp_process_auto_increment_tracknumber_variable(const char *tag_variable_start,
  const char *end_paranthesis, tags_parser_utils *tpu, splt_state *state, int *error);

static void splt_tp_check_ambigous_next_position(
  const char *tag_variable_start, tags_parser_utils *tpu);
static void splt_tp_process_original_tags_variable(
  tags_parser_utils *tpu, splt_state *state, int *error, int set_original_tags);
...
static void splt_tp_look_for_all_tags_char(
  const char *tags, tags_parser_utils *tpu, splt_state *state);
static void splt_tp_tpu_increment_tags_field_counter(tags_parser_utils *tpu, int tags_field);
...
static void splt_tp_tpu_set_tracknumber(
  tags_parser_utils *tpu, const char *tracknumber, int *error);
static const char *splt_tp_tpu_get_tracknumber(tags_parser_utils *tpu);
static void splt_tp_tpu_set_tags_value(
  tags_parser_utils *tpu, int tags_field, const void *tag_value);
static void splt_tp_set_track_from_parsed_tracknumber(
...
static char *splt_tp_parse_tag_word(
  const char *cur_pos, const char *end_paranthesis, int *ambiguous, int *error)
{
  char *word = NULL;
  char *word_end = NULL;
  char *word_end2 = NULL;
  ...
  if ((word_end = strchr(cur_pos, ',')))
  {
    ...
    if (*word_end == ',')
    {
      if (*(word_end + 1) != '@') { *ambiguous = SPLT_TRUE; }
    }
  }
  else { word_end = strchr(cur_pos, ']'); }
...
{
  if (tags == NULL) { return SPLT_FALSE; }
  ...
  while ((tpu->position = strchr(tpu->position, '[')))
  {
    ...
    splt_tu_append_tags_to_state(state, tpu->current_tags, !tpu->original_tags_found,
      tpu->original_tags_value, SPLT_FALSE, error);
    if (*error < 0) { goto end; }
    ...
    else if (tpu->we_had_all_tags && !tpu->original_tags_found)
    {
      ...
      splt_tu_set_new_tags_where_current_tags_are_null(
        state, tpu->current_tags, tpu->all_tags, index, error);
      if (*error < 0) { goto end; }
    }
...
{
  if ((tpu->title_counter > 1) || (tpu->artist_counter > 1) || (tpu->album_counter > 1) ||
      (tpu->performer_counter > 1) || (tpu->year_counter > 1) || (tpu->comment_counter > 1) ||
      (tpu->tracknumber_counter > 1) || (tpu->genre_counter > 1))
  {
...
static char *splt_tp_find_tag_start(const char *position, const char *end_paranthesis)
{
  ...
  if (!comma_or_open_bracket_found) { return NULL; }
  ...
  const char *tags, tags_parser_utils *tpu, splt_state *state, int *error)
{
  ...
  {
    if (tag_start >= end_paranthesis) { break; }
    ...
    if (tpu->position == tag_start + 1) { tpu->position++; }
  }
  ...
  tags_parser_utils *tpu, splt_state *state, int *error)
{
  ...
  if (tracknumber)
  {
    ...
    int track = 1;
    if (is_number) { track = atoi(tracknumber); }
  ...
  const char *end_paranthesis, tags_parser_utils *tpu, splt_state *state, int *error)
{
  ...
  switch (tag_variable)
  {
    ...
    case 'O':
      ...
      break;
    case 'a':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_ARTIST, tpu, state, error);
      break;
    case 'p':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_PERFORMER, tpu, state, error);
      break;
    case 'b':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_ALBUM, tpu, state, error);
      break;
    case 't':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_TITLE, tpu, state, error);
      break;
    case 'c':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_COMMENT, tpu, state, error);
      break;
    case 'g':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_GENRE, tpu, state, error);
      break;
    case 'y':
      splt_tp_process_str_tags_variable(
        tag_variable_start, end_paranthesis, SPLT_TAGS_YEAR, tpu, state, error);
      break;
    case 'n':
      splt_tp_process_tracknumber_variable(tag_variable_start, end_paranthesis, tpu, state, error);
      break;
    case 'N':
      splt_tp_process_auto_increment_tracknumber_variable(
        tag_variable_start, end_paranthesis, tpu, state, error);
      break;
    default:
      tpu->ambigous = SPLT_TRUE;
      break;
...
{
  char *tag_value =
    splt_tp_parse_tag_word(tag_variable_start, end_paranthesis, &tpu->ambigous, error);
  if (*error < 0) { return; }
  ...
  const char *end_paranthesis, tags_parser_utils *tpu, splt_state *state, int *error)
{
  ...
  if (tpu->set_all_tags) { tpu->auto_increment_tracknumber = SPLT_TRUE; }
}
...
static void splt_tp_tpu_set_tracknumber(tags_parser_utils *tpu, const char *tracknumber, int *error)
{
  ...
  const char *tag_variable_start, tags_parser_utils *tpu)
{
  ...
  if ((next_position != ',') && (next_position != ']')) { tpu->ambigous = SPLT_TRUE; }
}

static void splt_tp_process_original_tags_variable(
  tags_parser_utils *tpu, splt_state *state, int *error, int set_original_tags)
{
  if (tpu->original_tags_found) { tpu->ambigous = SPLT_TRUE; }
  ...
  if (splt_tp_tpu_has_one_current_tag_set(tpu)) { tpu->ambigous = SPLT_TRUE; }
...
static char *splt_tp_look_for_end_paranthesis(tags_parser_utils *tpu)
{
  ...
  if (!end_paranthesis) { tpu->ambigous = SPLT_TRUE; }
  else
  {
    char after_end_paranthesis = *(end_paranthesis + 1);
    if ((after_end_paranthesis != '[') && (after_end_paranthesis != '%') &&
  ...
  const char *tags, tags_parser_utils *tpu, splt_state *state)
{
  ...
  {
    splt_o_set_int_option(state, SPLT_OPT_ALL_REMAINING_TAGS_LIKE_X, tpu->tags_counter);
...
static void splt_tp_tpu_set_all_tags_field(
  tags_parser_utils *tpu, int tags_field, const void *tag_value)
{
  if (tpu->set_all_tags) { splt_tu_set_field_on_tags(tpu->all_tags, tags_field, tag_value); }
}

static void splt_tp_tpu_set_current_tags_field(
  tags_parser_utils *tpu, int tags_field, const void *tag_value)
{
