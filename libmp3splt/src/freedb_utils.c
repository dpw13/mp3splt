...
static int splt_fu_append_first_result(splt_freedb_results *res, const char *album_name);
static int splt_fu_append_next_result(splt_freedb_results *res, const char *album_name);
static int splt_fu_append_first_revision(splt_freedb_one_result *prev, const char *album_name);
static int splt_fu_append_next_revision(splt_freedb_one_result *prev, const char *album_name);
...
int splt_fu_freedb_append_result(splt_state *state, const char *album_name, int revision)
{
  ...
  if (album_name == NULL) { return SPLT_OK; }

  if (res->number == 0) { return splt_fu_append_first_result(res, album_name); }

  if (revision != -1) { return splt_fu_append_next_result(res, album_name); }

  splt_freedb_one_result *prev = &res->results[res->number - 1];

  if (prev->revision_number == 0) { return splt_fu_append_first_revision(prev, album_name); }

  return splt_fu_append_next_revision(prev, album_name);
}

int splt_fu_freedb_get_found_cds(splt_state *state) { return state->fdb.cdstate->foundcd; }
...
const char *splt_fu_freedb_get_disc_category(splt_state *state, int index)
{
  ...
  if ((index >= 0) && (index < cdstate->foundcd)) { return cdstate->discs[index].category; }
  else
  {
...
const char *splt_fu_freedb_get_disc_id(splt_state *state, int index)
{
  ...
  if ((index >= 0) && (index < cdstate->foundcd)) { return cdstate->discs[index].discid; }
  else
  {
    splt_e_error(SPLT_IERROR_INT, __func__, index, NULL);
    return NULL;
...
static int splt_fu_append_first_result(splt_freedb_results *res, const char *album_name)
{
  ...
  if (res->results == NULL) { return SPLT_ERROR_CANNOT_ALLOCATE_MEMORY; }
...
static int splt_fu_append_first_revision(splt_freedb_one_result *prev, const char *album_name)
{
  ...
  if (prev->revisions == NULL) { return SPLT_ERROR_CANNOT_ALLOCATE_MEMORY; }
