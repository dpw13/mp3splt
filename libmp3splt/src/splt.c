...
static long splt_s_real_split(
  double splt_beg, double splt_end, int save_end_point, int *error, splt_state *state)
{
  ...
  if (*error >= 0)
  {
    ...
    if (new_sec_end_point == -1.0) { new_end_point = LONG_MAX; }
    else { new_end_point = splt_co_time_to_long_ceil(new_sec_end_point); }
...
static long splt_s_split(splt_state *state, int first_splitpoint, int second_splitpoint, int *error)
{
  ...
  if (get_error == SPLT_OK)
  {
    ...
    if (*error >= 0)
    {
      ...
      if (split_begin != split_end)
      {
        ...
        //LONG_MAX == EOF
        if (split_end == LONG_MAX) { splt_end = -1; }
        else
        {
...
void splt_s_multiple_split(splt_state *state, int *error)
{
  ...
  int save_end_point = SPLT_TRUE;
  if (splt_o_get_long_option(state, SPLT_OPT_OVERLAP_TIME) > 0) { save_end_point = SPLT_FALSE; }
  ...
  while (i < number_of_splitpoints - 1)
  {
    ...
    if (!splt_t_split_is_canceled(state))
    {
      ...
      if ((*error < 0) || (*error == SPLT_OK_SPLIT_EOF)) { break; }
    }
    else
    {
  ...
  for (i = 0; i < splt_array_length(new_end_points); i++)
  {
    ...
    splt_sp_set_splitpoint_value(
      state, splt_il_pair_first(index_end_point), splt_il_pair_second(index_end_point));
      ...
      if (!splt_t_split_is_canceled(state))
      {
        ...
        if (*error >= 0)
        {
          ...
          int split_result = splt_p_simple_split(state, final_fname,
            (off_t)state->serrors->serrors_points[i], (off_t)state->serrors->serrors_points[i + 1]);

          //automatically set progress callback to 100%
          splt_c_update_progress(state, 1.0, 1.0, 1, 1, 1);

          if (split_result >= 0) { *error = SPLT_SYNC_OK; }
          else { *error = split_result; }
...
static double splt_s_get_real_end_time_splitpoint(
  splt_state *state, int current_split, long total_time)
{
  long overlapped_time = splt_sp_overlap_time(state, current_split + 1);
  double overlapped_end = -1;
  if (overlapped_time == LONG_MAX) { return overlapped_end; }

  overlapped_end = (double)((double)overlapped_time / 100.0);

  if (total_time <= 0) { return overlapped_end; }
...
static void splt_s_split_by_time(
  splt_state *state, int *error, double split_time_length, int number_of_files)
{
  char *final_fname = NULL;
  int j = 0, tracks = 1;
  double begin = 0.f;
    ...
    if (err >= 0)
    {
      ...
      do {
        if (!splt_t_split_is_canceled(state))
        {
          ...
          splt_sp_set_splitpoint_value(state, current_split, splt_co_time_to_long_ceil(begin));
          ...
          double new_sec_end_point =
            splt_p_split(state, final_fname, begin, real_end_splitpoint, error, save_end_point);
          ...
          //get out if error
          if ((*error == SPLT_MIGHT_BE_VBR) || (*error == SPLT_OK_SPLIT_EOF) || (*error < 0))
          {
            tracks = 0;
          }

          if (*error == SPLT_ERROR_BEGIN_OUT_OF_FILE) { j--; }
  ...
  else { *error = SPLT_ERROR_NEGATIVE_TIME_SPLIT; }
}
...
void splt_s_equal_length_split(splt_state *state, int *error)
{
  ...
  if (total_time > 0)
  {
    int number_of_files = splt_o_get_int_option(state, SPLT_OPT_LENGTH_SPLIT_FILE_NUMBER);
  ...
  if (*error == SPLT_TIME_SPLIT_OK) { *error = SPLT_LENGTH_SPLIT_OK; }
}
...
int splt_s_set_trim_silence_splitpoints(splt_state *state, int *error)
{
  ...
  for (i = 1; i < found + 1; i++)
  {
    ...
    if (i > 1 && temp_silence_pos < previous) { temp_silence_pos = LONG_MAX; }
...
int splt_s_set_silence_splitpoints(splt_state *state, int *error)
{
  ...
  else { snprintf(remove_str, 128, _("NO")); }
  ...
  else { snprintf(auto_user_str, 128, _("Auto")); }
  ...
  if (!splt_o_get_int_option(state, SPLT_OPT_QUIET_MODE))
  {
    ...
    {
      char *min_track_join = splt_su_get_formatted_message(state, ", %s: %.2f", "Min track join",
  ...
  if (*error >= 0)
  {
    if (!read_silence_from_logs) { splt_c_put_info_message_to_client(state, "\n"); }
    ...
    {
      found++;
      if ((number_tracks > 0) && (number_tracks < SPLT_MAXSILENCE))
      {
        if (number_tracks < found) { found = number_tracks; }
      }
      ...
      for (i = 1; i < found; i++)
      {
        ...
        if (splt_o_get_int_option(state, SPLT_OPT_PARAM_REMOVE_SILENCE))
        {
          ...
          {
            long adjusted_silence_left =
              splt_co_time_to_long(temp->begin_position + keep_silence_right);
            long adjusted_silence_right =
              splt_co_time_to_long(temp->end_position - keep_silence_left);

            append_error =
              splt_sp_append_splitpoint(state, adjusted_silence_left, NULL, SPLT_SKIPPOINT);
            if (append_error < 0)
            {
              *error = append_error;
              found = i;
      ...
      {
        long total_time = splt_t_get_total_time(state);
        if (total_time <= 0) { total_time = LONG_MAX; }

        //last splitpoint, end of file
        append_error = splt_sp_append_splitpoint(state, total_time, NULL, SPLT_SPLITPOINT);
    ...
    else { *error = SPLT_SPLIT_CANCELLED; }
...
void splt_s_silence_split(splt_state *state, int *error)
{
  ...
  if (*error >= 0)
  {
    ...
    else { *error = SPLT_NO_SILENCE_SPLITPOINTS_FOUND; }
  }
}
...
static splt_code splt_s_open_full_log_filename(splt_state *state)
{
  ...
  if (!fname || fname[0] == '\0') { return SPLT_OK; }
...
static void splt_s_close_full_log_filename(splt_state *state)
{
  if (!state->full_log_file_descriptor) { return; }
