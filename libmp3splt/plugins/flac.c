...
static void splt_flac_get_info(
  splt_state *state, FILE *file_input, const char *input_filename, splt_code *error);
static splt_flac_state *splt_flac_info(
  FILE *in, splt_state *state, const char *input_filename, splt_code *error);
...
double splt_pl_split(splt_state *state, const char *output_fname, double begin_point,
  double end_point, int *error, int save_end_point)
{
  ...
  if (splt_o_get_int_option(state, SPLT_OPT_TAGS) == SPLT_NO_TAGS || tags_to_write == NULL)
  {
    flac_tags = NULL;
  }

  splt_flac_fr_read_and_write_frames(state, flacstate->fr, flacstate->metadatas, flac_tags,
    tags_to_write, output_fname, begin_point, end_point, save_end_point,
    flacstate->streaminfo.min_blocksize, flacstate->streaminfo.max_blocksize,
    flacstate->streaminfo.bits_per_sample, flacstate->streaminfo.sample_rate,
    flacstate->streaminfo.channels, flacstate->streaminfo.min_framesize,
    flacstate->streaminfo.max_framesize, flacstate->off, error);
...
void splt_pl_import_internal_sheets(splt_state *state, splt_code *error)
{
  ...
  splt_cc_put_filenames_from_tags(state, track_number, error, our_tags, SPLT_FALSE, SPLT_FALSE);
  ...
  splt_state *state, FILE *file_input, const char *input_filename, splt_code *error)
{
  ...
  if (!splt_o_messages_locked(state))
  {
    ...
    snprintf(flac_infos, 1023, _(" info: Native FLAC - %u Hz - %d channels of %d bits"),
  ...
  FILE *in, splt_state *state, const char *input_filename, splt_code *error)
{
  ...
  double total_time =
    ((double)flacstate->streaminfo.total_samples / (double)flacstate->streaminfo.sample_rate) *
    100.0;
