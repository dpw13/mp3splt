...
static void print_with_spaces_after(
  const char *printed_value, int print_carriage_return, FILE *file, main_data *data)
{
  ...
  if (print_carriage_return) { fprintf(file, "%s%s\r", value, temp); }
  else
  {
    ...
    else { fprintf(file, "%s%s", value, temp); }
  }
...
void put_progress_bar(splt_progress *p_bar, void *user_data)
{
  ...
  switch (mp3splt_progress_get_type(p_bar))
  {
    case SPLT_PROGRESS_PREPARE:
      snprintf(progress_text, 2047, _(" preparing \"%s\" (%d of %d)"), filename_shorted,
        mp3splt_progress_get_current_split(p_bar), mp3splt_progress_get_max_splits(p_bar));
      break;
    case SPLT_PROGRESS_CREATE:
      snprintf(progress_text, 2047, _(" creating \"%s\" (%d of %d)"), filename_shorted,
        mp3splt_progress_get_current_split(p_bar), mp3splt_progress_get_max_splits(p_bar));
      break;
    case SPLT_PROGRESS_SEARCH_SYNC:
      snprintf(progress_text, 2047, _(" searching for sync errors..."));
      break;
  ...
  if (percent_progress <= 0.01) { snprintf(printed_value, 2047, " [ - %%] %s", progress_text); }
  else { snprintf(printed_value, 2047, " [ %.2f %%] %s", percent_progress * 100, progress_text); }
...
void print_version(FILE *std)
{
  ...
  fprintf(std,
    PACKAGE_NAME " " VERSION " (" MP3SPLT_DATE
                 ") -"
                 " %s libmp3splt %s\n",
    _("using"), version);
  fflush(std);
...
{
  fprintf(
    std, "\t" MP3SPLT_AUTHOR1 " " MP3SPLT_EMAIL1 "\n\t" MP3SPLT_AUTHOR2 " " MP3SPLT_EMAIL2 "\n");
  fflush(std);
}
